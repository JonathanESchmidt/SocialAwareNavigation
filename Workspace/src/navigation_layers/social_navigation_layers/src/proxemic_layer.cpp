  ...
  for (p_it = transformed_people_.begin(); p_it != transformed_people_.end(); ++p_it)
  {
    ...
    for (int i = start_x; i < end_x; i++)
    {
      for (int j = start_y; j < end_y; j++)
      {
        ...
        unsigned char avalue = 255; //Set costmap pixel value
        costmap->setCost(i + dx, j + dy, std::max(avalue, old_cost)); //Adjustments to add an obstacle in the center of inflation for costmap_converter to pick up (Gr 667)
      }
    }
  }
}
