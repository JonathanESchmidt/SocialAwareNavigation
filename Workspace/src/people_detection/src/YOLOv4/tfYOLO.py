...
class tfYOLOv4():
    def __init__(self, width = 1024, height = 768, path = "/yolov4.h5"):
    ...
    def predictOPENCV(self, cvimg):
        
        self.image =tf.reverse(cvimg,axis=[-1])#convert from bgr to rgb
        #self.image = tf.io.decode_image(self.image) # no need to decode as it is not saved as a specific file format
        self.image = tf.image.resize(self.image, (self.HEIGHT, self.WIDTH))
        self.images = tf.expand_dims(self.image, axis=0) / 255
...
#     testModel.predict('/home/tristan/Repositories/SocialAwareNavigation/Workspace/src/people_detection/src/YOLOv4/Testimage.jpeg')
